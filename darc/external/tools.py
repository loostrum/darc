...
def group_dm_time_beam(fdir, fnout=None, trigname='cand'):
    ...
    for fn in flist:
        ...
        try:
            sig_cut, dm_cut, tt_cut, ds_cut, ind_full = get_triggers(fn, sig_thresh=8.0,
...
if __name__ == '__main__':
    ...
    try:
        par_1a, par_2a, par_match_arra, ind_misseda, ind_matcheda = SNRTools.compare_snr(
            fn_1, fn_2,
        ...
        if options.plot_both is True:
            par_1b, par_2b, par_match_arrb, ind_missedb, ind_matchedb = SNRTools.compare_snr(
                fn_2, fn_1,
                dm_min=options.dm_min,
                dm_max=options.dm_max, save_data=False,
                sig_thresh=options.sig_thresh,
    ...
    if options.mk_plot is True:
        ...
        if options.truthfile is not None:
            par_1, par_1_truth, par_match_1, ind_misseda, ind_matched1 = SNRTools.compare_snr(
                fn_1, options.truthfile,
                dm_min=options.dm_min,
                dm_max=options.dm_max, save_data=False,
                sig_thresh=options.sig_thresh,
                t_window=options.t_window,
            ...
            par_2, par_2_truth, par_match_2, ind_missedb, ind_matched2 = SNRTools.compare_snr(
                fn_2, options.truthfile,
                dm_min=options.dm_min,
                dm_max=options.dm_max, save_data=False,
                sig_thresh=options.sig_thresh,
                t_window=options.t_window,
